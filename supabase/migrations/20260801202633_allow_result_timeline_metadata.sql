-- Result entry accepts the normalized event metadata used by the timeline.
-- The result transaction remains the only path that may complete a match.
...
begin
  ...
  select count(*) into event_count from jsonb_array_elements(event_rows);
  ...
  insert into public.match_events (
    team_id, match_id, player_id, type, minute, stoppage_time, notes
  )
  select
    target_team_id,
    target_match_id,
