-- Task 013: preserve the normalized match-event timeline with optional
-- stoppage time and notes. The event type set and ownership model remain
-- intentionally unchanged.

alter table public.match_events
  add column stoppage_time integer not null default 0,
  add column notes text;

alter table public.match_events
  add constraint match_events_stoppage_time_check
    check (stoppage_time >= 0 and stoppage_time <= 2147483647),
  add constraint match_events_notes_length_check
...
begin
  ...
  if exists (
    ...
    where keys.key not in ('type', 'player_id', 'minute', 'stoppage_time', 'notes')
  ) then
  ...
  if exists (
    select 1
    from jsonb_array_elements(event_rows) as rows(value)
    where rows.value ? 'stoppage_time'
      and rows.value->'stoppage_time' is not null
      and (
        jsonb_typeof(rows.value->'stoppage_time') <> 'number'
        or rows.value->>'stoppage_time' !~ '^[0-9]+$'
        or (rows.value->>'stoppage_time')::numeric > 2147483647
      )
  ) then
    raise exception 'invalid_event_payload' using errcode = '22023';
  end if;
  ...
  if exists (
    select 1
    from jsonb_array_elements(event_rows) as rows(value)
    where rows.value ? 'notes'
      and rows.value->'notes' is not null
      and (
        jsonb_typeof(rows.value->'notes') <> 'string'
        or char_length(rows.value->>'notes') > 500
      )
  ) then
    raise exception 'invalid_event_payload' using errcode = '22023';
  end if;
  ...
  select
    ...
    coalesce((rows.value->>'stoppage_time')::integer, 0),
    nullif(btrim(rows.value->>'notes'), '')
  from jsonb_array_elements(event_rows) as rows(value);
